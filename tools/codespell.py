...
_data_root = os.path.join(os.path.dirname(os.path.dirname(os.path.realpath(__file__))), "config","data")
...
class Summary:
    ...
    def __str__(self) -> str:
        ...
        result = []
        for key in keys:
            result.append(key+""+" "*(15-len(key))+str(self.summary[key]))
        return "\n".join(result)
...
class FileOpener:
    ...
    def open_with_chardet(self, filename: str) -> Tuple[List[str], str]:
        ...
        try:
            ...
            print("ERROR: Could not detect encoding: {}".format(filename), file=sys.stderr)
            raise
        except LookupError:
            print(
                "ERROR: Don't know how to handle encoding {}: {}".format(encoding,filename),
                file=sys.stderr,
            )
            raise
        else:
    ...
    def open_with_internal(self, filename: str) -> Tuple[List[str], str]:
        ...
        for encoding in encodings:
            if first_try:
                ...
                print('WARNING: Trying next encoding "{}"'.format(encoding), file=sys.stderr)
            with open(filename, encoding=encoding, newline="") as f:
                try:
                    ...
                    if not self.quiet_level & QuietLevels.ENCODING:
                        print(
                            'WARNING: Cannot decode file using encoding "{}": '.format(encoding),
                            filename,
                            file=sys.stderr,
                        )
                else:
...
def parse_options(
    ...
    builtin_opts = "\n- ".join(
        [""] + ["{} {}".format(d[0],d[1]) for d in _builtin_dictionaries]
    )
    parser.add_argument(
        "--builtin",
        dest="builtin",
        ...
        if sys.version_info >= (3, 11):
            ...
            try:
                ...
                if tomllib_raise_error:
                    raise ImportError(
                        "tomllib or tomli are required to read pyproject.toml "
                        "but could not be imported, got: {}".format(e)
                    ) from None
...
def ask_for_word_fix(
    ...
    line_ui = (
        str({line[:match.start()]}),
        "{}{}{}".format(colors.WWORD,wrongword,colors.DISABLE),
        str({line[match.end():]})
    )
    ...
    if misspelling.fix and interactivity & 1:
        ...
        while not r:
            print("{}\t{} ==> {} (Y/n) ".format(line_ui,wrongword,fixword), end="", flush=True)
        ...
        while not r:
            print("{} Choose an option (blank for none): ".format(line_ui), end="")
            for i, o in enumerate(opt):
                fixword = fix_case(wrongword, o)
                print(" {}) {}".format(i,fixword), end="")
...
def print_context(
    ...
    for i in range(index - context[0], index + context[1] + 1):
        if 0 <= i < len(lines):
            str1 = '>' if i == index else ':'
            print("{} {}".format(str1,lines[i].rstrip()))
    ...
    if filename == "-":
        ...
        if options.check_filenames:
            for word in extract_words(filename, word_regex, ignore_word_regex):
                ...
                cfilename = str(colors.FILE)+str(filename)+str(colors.DISABLE)
                cwrongword = str(colors.WWORD)+str(word)+str(colors.DISABLE)
                crightword = str(colors.FWORD)+str(fixword)+str(colors.DISABLE)

                reason = misspellings[lword].reason
                if reason:
                    if options.quiet_level & QuietLevels.DISABLED_FIXES:
                        continue
                    creason = "  | "+str(colors.FILE)+str(reason)+str(colors.DISABLE)
                else:
                ...
                print("{}: {} ==> {}{}".format(cfilename,cwrongword,crightword,creason))
        ...
        try:
            ...
            print("WARNING: {}: {}".format(e.strerror,filename), file=sys.stderr)
            return bad_count
        except OSError:
            return bad_count
        ...
        if not text:
            if not options.quiet_level & QuietLevels.BINARY_FILE:
                print("WARNING: Binary file: {}".format(filename), file=sys.stderr)
            return bad_count
        try:
    ...
    for i, line in enumerate(lines):
        ...
        for match in check_matches:
            ...
            if lword in misspellings:
                ...
                cfilename = str(colors.FILE)+str(filename)+str(colors.DISABLE)
                cline = str(colors.FILE)+str(i + 1)+str(colors.DISABLE)
                cwrongword = str(colors.WWORD)+str(word)+str(colors.DISABLE)
                crightword = str(colors.FWORD)+str(fixword)+str(colors.DISABLE)
                ...
                if filename != "-":
                    print(
                        "{}:{}: {} ".format(cfilename,cline,cwrongword),
                        "==> {}{}".format(crightword,creason)
                    )
                else:
                    print(
                        "{}: {}\n\t{} ".format(cline,line.strip(),cwrongword),
                        "==> {crightword}{creason}".format(crightword=crightword,creason=creason)
                    )
    ...
    if changed:
        if filename == "-":
            ...
            if not options.quiet_level & QuietLevels.FIXES:
                print(
                    str(colors.FWORD)+"FIXED:"+str(colors.DISABLE)+" "+str(filename),
                    file=sys.stderr,
                )
...
def main(*args: str) -> int:
    ...
    if not options.quiet_level & QuietLevels.CONFIG_FILES:
        ...
        for ifile, cfg_file in enumerate(used_cfg_files, start=1):
            print("    {ifile}: {cfg_file}".format(ifile=ifile,cfg_file=cfg_file))
    ...
    try:
        ...
        print('ERROR: invalid --regex "{}" ({})'.format(word_regex,e), file=sys.stderr)
        parser.print_help()
        return EX_USAGE
    ...
    if options.ignore_regex:
        try:
            ...
            print(
                'ERROR: invalid --ignore-regex "{}" ({})'.format(options.ignore_regex,e),
                file=sys.stderr,
            )
            parser.print_help()
            return EX_USAGE
    ...
    for ignore_words_file in ignore_words_files:
        if not os.path.isfile(ignore_words_file):
            print(
                "ERROR: cannot find ignore-words file: {}".format(ignore_words_file),
                file=sys.stderr,
            )
            parser.print_help()
            return EX_USAGE
    ...
    try:
        ...
        print(
            'ERROR: invalid --uri-regex "{}" ({})'.format(uri_regex,e),
            file=sys.stderr,
        )
        parser.print_help()
        return EX_USAGE
    ...
    for dictionary in dictionaries:
        if dictionary == "-":
                ...
                for builtin in _builtin_dictionaries:
                    if builtin[0] == u:
                        use_dictionaries.append(
                            os.path.join(_data_root, "dictionary{}.txt".format(builtin[2]))
                        )
                        break
                else:
                    print(
                        "ERROR: Unknown builtin dictionary: {}".format(u),
                        file=sys.stderr,
                    )
                    parser.print_help()
                    return EX_USAGE
            ...
            if not os.path.isfile(dictionary):
                print(
                    "ERROR: cannot find dictionary file: {dictionary}".format(dictionary=dictionary),
                    file=sys.stderr,
                )
                parser.print_help()
                return EX_USAGE
